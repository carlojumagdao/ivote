...
class editFormEncoder {
	...
	private function element_single_line_text($field)
	{
	    ...
	    if($id == "member_id"){
	    	...
	    	$html .= sprintf('<input type="text" name="%s" id="%s" class="form-control validate %s" value="%s">', $id, $id, $required,$this->strMemFname);
	    } else if($id == "middle_name"){
	    	$html .= sprintf('<input type="text" name="%s" id="%s" class="form-control validate %s" value="%s">', $id, $id, $required,$this->strMemMname);
	    } else if($id == "last_name"){
	    	$html .= sprintf('<input type="text" name="%s" id="%s" class="form-control validate %s" value="%s">', $id, $id, $required,$this->strMemLname);
	    } else if($id == "email"){
	    	$html .= sprintf('<input type="text" name="%s" id="%s" class="form-control validate %s" value="%s">', $id, $id, $required,$this->strMemEmail);
	    } 
	    else {
	    	$intCounter = 0;
	    	$blHasData = false;
	    	foreach ($this->arrFieldName as $value) {
	    		if($id == "$value"){
			    	$html .= sprintf('<input type="text" name="%s" id="%s" class="validate %s" value="%s">', $id, $id, $required,$this->arrFieldData[$intCounter]);
			    	$blHasData = true;
			    	break;
			    }
			    $intCounter++;	    	
	...
	private function element_number($field)
	{
		...
		$html = '<div class="form-group">';
		$html .= $this->make_label($id, $field->title, $required);
	    $html .= sprintf('<input type="number" name="%s" id="%s" class="form-control %s" value="%s">', $id, $id, $required,$numvalue);
	  	$html .= '</div>';
	  	//echo "$id<br/>";
	  	return $html;
	}
	...
	private function element_email($field)
	{
		...
		$html = '<div class="form-group">';
		$html .= $this->make_label($id, $field->title, $required);
	    $html .= sprintf('<input type="email" name="%s" id="%s" class="form-control" value="%s" %s>', $id, $id, $this->strMemEmail, $required);
	  	$html .= '</div>';
	  	//echo "$id<br/>";
	  	return $html;
	}
	...
	private function element_paragraph_text($field)
	{
		...
		$html .= $this->make_label($id, $field->title, $required);
	    $html .= sprintf('<textarea id="%s" name="%s" class="materialize-textarea %s" rows="3" value="%s" >%s</textarea>', $id, $id, $required, $required,$txtvalue);
	  	$html .= '</div>';
	  	return $html;
	}
